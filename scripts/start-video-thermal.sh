...
if [ "$THERMALCAMERA" = "boson640" ] || [ "$THERMALCAMERA" = "boson320" ] ; then
    ...
    for device in $video_devices; do
        ...
        if [[ "$device_name" == *"EchoTherm"* || "$device_name" == *"Dummy video"* ]]; then        
            ...
            gst-client pipeline_create thermalSrc v4l2src device=$device io-mode=mmpa ! queue ! videocrop top=0 left=0 right=0 bottom=0 name=thermalZoom ! videoscale ! video/x-raw,width=320,height=256 ! v4l2h264enc extra-controls="controls,repeat_sequence_header=1,h264_profile=1,h264_level=11,video_bitrate=${SCALED_THERMAL_BITRATE},h264_i_frame_period=30,h264_minimum_qp_value=10" name=thermalEncoder ! "video/x-h264,level=(string)4" ! rtph264pay config-interval=1 pt=96 ! interpipesink name=thermalSrc           
            # original pipeline
